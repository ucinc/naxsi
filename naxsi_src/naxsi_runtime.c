...
#define dummy_error_fatal(ctx, r, ...) do {	\
    if (ctx) ctx->block = 1;						\
    ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,  "XX-******** NGINX DUMMY INTERNAL ERROR ********"); \
...
*/
int	ngx_http_process_basic_rule_buffer(ngx_str_t *str,
					   ngx_http_rule_t *rl,
					   ngx_int_t	*nb_match)
...
int	ngx_http_dummy_is_rule_whitelisted(ngx_http_request_t *req, ngx_http_dummy_loc_conf_t *cf, 
					   ngx_http_rule_t *r, ngx_str_t *name, enum DUMMY_MATCH_ZONE zone)
{
  ngx_int_t			k;
	...
	for (z = 0; dr[i]->wl_id[z] > 0; z++)
	  {
		...
		//		if (dr[i]->br && dr[i]->br->body, dr[i]->br->args, dr[i]->br->url, dr[i]->br->headers);
		if (dr[i]->br && 
...
#ifdef whitelist_debug
		    ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, 
				  "Rule [%d] was disabled, whitelist.", r->rule_id);
#endif
		    return (1);
		  }
	      }
  ...
  if (b)
    {
#ifdef whitelist_debug
      ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, "Name match in zone ARGS");
#endif
      for (cl = b->whitelist_locations->elts, i = 0; i < b->whitelist_locations->nelts; i++)
	{
#ifdef whitelist_debug
	  ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, 
  ...
  if (cf->wlr_url_hash && cf->wlr_url_hash->size > 0)
    {
      ...
      b = (ngx_http_whitelist_rule_t*) ngx_hash_find(cf->wlr_url_hash, k, 
						     (u_char*) req->uri.data, req->uri.len);
      if (b)
	{
#ifdef whitelist_debug
	  ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, 
			"This URL has some whitelist !");
	  //then verify if it is on this specific argument context, and on this rule ID
	  ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, 
	  ...
	  for (cl = b->whitelist_locations->elts, i = 0; i < b->whitelist_locations->nelts; i++)
...
#endif
	      /*
	      ** Go through the list of location for the whitelist, as the URL is the same.
	      ** First, check for "generic" zones, like :
	      ** $URL:/bar|ARGS
	      ** $URL:/bar|BODY
	      ** ...
	      */
	      if ( (cl[i].args && (r->br->args_var || r->br->args)) ||
		   (cl[i].url && r->br->url) ||
		   (cl[i].body && (r->br->body_var || r->br->body)) ||
		   (cl[i].headers && (r->br->headers_var || r->br->headers)) )
		{
	      ...
	      /* 
	      ** if we have a specific white list on one name, either in headers,args or body
	      ** and that the rule has matched on the whole context (body,args,headers) OR 
	      ** the rule was matching on a specific [name] from one of those contexts, check
	      ** if the name are the sames :)
	      */
	      if ( (cl[i].body_var && (r->br->body_var || r->br->body)) || 
		   (cl[i].headers_var && (r->br->headers_var || r->br->headers)) || 
		   (cl[i].args_var && (r->br->args_var || r->br->args)) )
		{
#ifdef whitelist_debug
		  ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, "zone-match [check names] (%V)", name);
#endif
		  if (cl[i].name->len == name->len &&
		      !strncmp((const char *)name->data, (const char *)cl[i].name->data, cl[i].name->len))
		    {
#ifdef whitelist_debug
		      ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, 
				    "WhiteListing4 rule %d on var [%V] at uri [%V] [NameMATCH]",
				    r->rule_id, name, &(req->uri));
#endif
		      tmp_ptr = cl[i].ids->elts;
		      for (z = 0; z < cl[i].ids->nelts; z++)
...
ngx_int_t  ngx_http_output_forbidden_page(ngx_http_request_ctx_t *ctx, ngx_http_request_t *r)
{
  ngx_int_t     rc, w;
  u_int		i;
  char		*fmt;
  ...
  if (ctx->matched)
	...
	rc += snprintf(0, 0, fmt_rm, i, "-----BODY|ARGS|HEADERS|URL----", i, mr[i].rule->rule_id, i, mr[i].name->len, mr[i].name->data);
    }
  else //it's a flag match, weird + big_request, generate on the fly matched rules.
    {
  ...
  if (ctx->matched)
      ...
      for (i = 0; i < ctx->matched->nelts; i++)
	{
	  ...
	  if (mr[i].url)
	    strcat(tmp_zone, "URL");
	  w += snprintf(fmt+w, rc, fmt_rm, i, tmp_zone, i, mr[i].rule->rule_id, i, mr[i].name->len, mr[i].name->data);
#ifdef whitelist_debug
	  ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, 
...
void	ngx_http_apply_rulematch_v_n(ngx_http_rule_t *r, ngx_http_request_ctx_t *ctx, ngx_http_request_t *req,
				     ngx_str_t *name, ngx_str_t *value, enum DUMMY_MATCH_ZONE zone, ngx_int_t nb_match)
{
  ...
  if (!cf || !ctx )
    return ;
  if (ngx_http_dummy_is_rule_whitelisted(req, cf, r, name, zone) == 1)
    return ;
  if (nb_match == 0)
    nb_match = 1;
  ...
  if (req->uri.len > 0)
    {
      ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, 
		    "[%V][%V] rule(%d) matched %d times on (%V)=(%V) [zones(%s,%s,%s,%s]", r->log_msg, &(req->uri), r->rule_id, nb_match, name, value,
  ...
  if (r->sc_tag)
	...
	ctx->special_scores = ngx_array_create(req->pool, 1, sizeof(ngx_http_special_score_t));
      else //search into the list for matching special score
	{
...
int ngx_http_spliturl_ruleset(ngx_pool_t *pool,
			      char	*str,
			      ngx_array_t *rules,
			      ...
			      enum DUMMY_MATCH_ZONE	zone)
{
  ...
  while (str < (orig+full_len) && *str)
	  ...
	  (eq && ev && eq > ev)) /*?foobar&bla=test*/
	{
      ...
      else if (!eq && ev) /* ?&&val | ?var&& | ?val& | ?&val | ?val&var */
	{
	  ctx->weird_request = 1;
      ...
      else /* should be normal like ?var=bar& ..*/
	{
	  ...
	  if (!eq)
	    {
	      dummy_error_fatal(ctx, req, "spliturl error : malformed url, possible attack [%s]", str);
	      return (1);
	    }
	  eq++;
...
#ifdef spliturl_ruleset_debug
	  ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0,
	  		"XX-check check [%V]=[%V]", &(name), &(val));
#endif
	  /* ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, */
	  /* 		"XX-(args) URL decoding changed data ? after [%V] len=%d", */
	  /* 		&(val), val.len); */
  ...
  for (i = 0; i < rules->nelts && (!ctx->block || cf->learning) ; i++) 
    {
      ...
      if (r[i].br->custom_location && name && name->len > 0)
	{
	  ...
	  for (z = 0; z < r[i].br->custom_locations->nelts; z++)
	    {
		  ...
		  !strncasecmp((const char *)name->data, (const char *) location[z].target.data, location[z].target.len))
		{
#ifdef basestr_ruleset_debug
		  ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0,
...
#endif
		      ngx_http_apply_rulematch_v_n(&(r[i]), ctx, req, name, value, zone, nb_match);
		    }
		}
	    }
	}
				       ...
				       u_int			 len)
{
  ...
  ** 2: at the very begining of the body. */
  ...
  while (idx < len)
	...
	break;
      if (src[idx] != '-' || src[idx+1] != '-' || //check if line starts with -- (pre-boundary stuff) 
	  ngx_strncmp(src+idx+2, boundary, boundary_len) || //and if it's really followed by a boundary
	  idx+boundary_len + 2 + 2  >= len ||  //and if it's not the last boundary of the buffer
	  src[idx+boundary_len+2] != '\r' || src[idx+boundary_len+3] != '\n') //and if it's followed by \r\n
	{
      ...
      ** <DATA>
      */
      if (ngx_strncasecmp(src+idx, (u_char *) "Content-disposition: form-data;", 30))
	{
	...
	{
	  filen_start += 10;
	  filen_end = ngx_strchr(filen_start, '"');
	  /* here add support :
      ...
      if (!end)
	{
      ...
      // don't treat files yet ...
      if (filen_start)
	{
	  ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, 
			"XX-ALERT: SKIPPING FILE (fname:%s)",
			filen_start);
	  ctx->big_request = 1;
				  ...
				  ngx_http_dummy_main_conf_t *main_cf)
{
...
#ifdef dummy_body_parse_debug
      ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, 
		    "[POST] REQUEST BODY IN CHAIN [%s] (len=%d)", full_body, full_body_len);
#endif
    }
  ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, 
  ...
  if (r->headers_in.content_length_n != full_body_len)
    {
      dummy_error_fatal(ctx, r, "content-len header (%d) mismatch actual len (%d)", 
			r->headers_in.content_length_n, full_body_len);
      return ;
    }
  /* x-www-form-urlencoded POSTs */
  if (!ngx_strncasecmp(r->headers_in.content_type->value.data, (u_char *)"application/x-www-form-urlencoded", 33))
    //33 = echo -n "application/x-www-form-urlencoded" | wc -c
    {
      ...
      ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, 
		    "XX-POST DATA [%V]", &tmp);
#endif
      if(ngx_http_spliturl_ruleset(r->pool, (char *)tmp.data, cf->body_rules, main_cf->body_rules, r, ctx, BODY))
	{
#ifdef post_heavy_debug
  ...
  else if (!ngx_strncasecmp(r->headers_in.content_type->value.data, (u_char *) "multipart/form-data", 19))
...
*/
void	ngx_http_dummy_uri_parse(ngx_http_dummy_main_conf_t *main_cf, ngx_http_dummy_loc_conf_t *cf, 
...
void	ngx_http_dummy_args_parse(ngx_http_dummy_main_conf_t *main_cf, ngx_http_dummy_loc_conf_t *cf, 
				  ngx_http_request_ctx_t *ctx, ngx_http_request_t *r)
{
  ngx_str_t			tmp;
  ...
  if(ngx_http_spliturl_ruleset(r->pool, (char *)tmp.data, cf->get_rules, main_cf->get_rules, r, ctx, ARGS))
    {
...
void	ngx_http_dummy_headers_parse(ngx_http_dummy_main_conf_t *main_cf, ngx_http_dummy_loc_conf_t *cf, 
				     ngx_http_request_ctx_t *ctx, ngx_http_request_t *r)
{
  ...
  for (i = 0; !ctx->block ; i++) 
      ...
      if (i >= part->nelts) 
	{
      ...
      if (cf->header_rules)
	ngx_http_basestr_ruleset_n(r->pool, &(h[i].key), &(h[i].value), cf->header_rules, r, ctx, HEADERS);
      if (main_cf->header_rules)
	ngx_http_basestr_ruleset_n(r->pool, &(h[i].key), &(h[i].value), main_cf->header_rules, r, ctx, HEADERS);
    }
  return ;
}
				  ...
				  ngx_http_request_t	 *r)
{
  ...
  if ((r->method == NGX_HTTP_POST || r->method == NGX_HTTP_PUT) && 
      (cf->body_rules || main_cf->body_rules) && //presence of body rules (POST/PUT rules)
      r->request_body && (!ctx->block || cf->learning)) //and the presence of data to parse
...
void	ngx_http_dummy_update_current_ctx_status(ngx_http_request_ctx_t	*ctx, ngx_http_dummy_loc_conf_t	*cf, ngx_http_request_t *r)
{
  ...
  if (cf->wlr_url_hash)
      ...
      if (b)
	{
	  ...
	  for (cl = b->whitelist_locations->elts, i = 0; i < b->whitelist_locations->nelts; i++)
	    {
	      /* check if a whitelist applies to current flags set by internals */
	      if (cl[i].flags)
		{
		  tmp_ptr = cl[i].ids->elts;
		  for (z = 0; z < cl[i].ids->nelts; z++)
...
#ifdef custom_score_debug
		      ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
		      		    "XX-MATCH WHITELIST ? %d / curr flags : weird:%d big:%d", cl[i].ids[z], ctx->weird_request, ctx->big_request);
#endif
		      
		      if (ctx->weird_request && tmp_ptr[z] == WEIRD_REQUEST_INTERNAL_RULE_ID)
			{
#ifdef custom_score_debug
			  ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
					"XX-MATCH WHITELIST !");
		      ...
		      if (ctx->big_request && tmp_ptr[z] == BIG_BODY_INTERNAL_RULE_ID)
			{
#ifdef custom_score_debug
			  ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
					"XX-MATCH WHITELIST !");
