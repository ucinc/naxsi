...
ngx_int_t  ngx_http_dummy_create_hashtables(ngx_http_dummy_loc_conf_t *dlc, ngx_conf_t *cf)
{
  unsigned int	i,z, x;
  ...
  ngx_array_t		*elements_uri, *elements_body, *elements_vars, *elements_headers;
  ...
  my_wlr = ngx_array_create(cf->pool, dlc->whitelist_rules->nelts, sizeof(ngx_http_whitelist_rule_t));
  if (!my_wlr)
    return (NGX_ERROR);
  for(i = 0; i < dlc->whitelist_rules->nelts; i++) 
    {
      ...
      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "rule %d is whitelisted somehow", rl[i].wl_id[0]);
#endif
	...
	{
#ifdef whitelist_heavy_debug
	  ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "Disabled rule [%d]", rl[i].wl_id[0]);
#endif
      ...
      for (z = 0; z < br->custom_locations->nelts; z++) 
	{
	  if (loc[z].specific_url)
	    {
#ifdef whitelist_heavy_debug
	      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "whitelist has URI %V", &(loc[z].target));
#endif
	      var_idx = z;
	      zone = URL;
	      break;
	  ...
	  if (loc[z].body_var)
	    {
#ifdef whitelist_heavy_debug
	      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "whitelist has body_var %V", &(loc[z].target));
#endif
	      var_idx = z;
	      zone = BODY;
	    }
	  if (loc[z].headers_var)
	    {
#ifdef whitelist_heavy_debug
	      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "whitelist has header_var %V", &(loc[z].target));
#endif
	      var_idx = z;
	      zone = HEADERS;
	    }
	  if (loc[z].args_var)
	    {
#ifdef whitelist_heavy_debug
	      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "whitelist has arg_var %V", &(loc[z].target));
#endif
	      var_idx = z;
	      zone = ARGS;
	    }
	...
	{
	  if (wlr[x].zone == zone && !strcmp((const char *)wlr[x].name->data, (const char *)loc[var_idx].target.data))
	    {
#ifdef whitelist_heavy_debug
	      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "Found whitelist with name %V and zone %d", &(loc[var_idx].target), zone);
#endif
	      wlr = &(wlr[x]);
	      found = 1;
	      break;
	...
	{
#ifdef whitelist_heavy_debug
	  ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "Created whitelist name %V zone %s", &(loc[var_idx].target),
			     zone == ARGS ? "ARGS" : zone == HEADERS ? "HEADERS" : zone == URL ? "URI" : zone == BODY ? "BODY" : "UNKNOWN");
#endif
      ...
      for (z = 0; z < br->custom_locations->nelts; z++) 
	{
	  ...
	  */
	  if (!(br->body || br->headers || br->url || br->args) && loc[z].specific_url)
	    {
#ifdef whitelist_heavy_debug
	  ...
	  if (br->body || br->headers || br->url || br->args)
	    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "Pushing to LOCATIONS [GENERIC ZONE]", &(loc[z].target));
	  else
	    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "Pushing to LOCATIONS %V", &(loc[z].target));
#endif
	  wl_loc = ngx_array_push(wlr->whitelist_locations);
	  if (!wl_loc)
	    return (NGX_ERROR);
	  ...
	  ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
			     ...
			     br->args, br->args_var, br->headers, br->headers_var, br->body, br->body_var,
  ...
  */
  ...
  elements_uri = ngx_array_create(cf->pool, my_hash_init_size, sizeof(ngx_hash_key_t));
  elements_headers = ngx_array_create(cf->pool, my_hash_init_size, sizeof(ngx_hash_key_t));
  elements_vars = ngx_array_create(cf->pool, my_hash_init_size, sizeof(ngx_hash_key_t));
  elements_body = ngx_array_create(cf->pool, my_hash_init_size, sizeof(ngx_hash_key_t));
  for(i = 0; i < my_wlr->nelts; i++)
    {
#ifdef whitelist_debug
      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "pushing for zone %s with name %V",
			 wlr[i].zone == BODY ? "BODY" : wlr[i].zone == HEADERS ? "HEADERS" : wlr[i].zone == ARGS ? "ARGS" :
	...
	return (NGX_ERROR);
      ...
      arr_node->key_hash  = ngx_hash_key_lc(wlr[i].name->data, wlr[i].name->len);
  ...
  if (elements_uri->nelts > 0)
    {
      ...
      if (ngx_hash_init(&hash_init, (ngx_hash_key_t*) elements_uri->elts, elements_uri->nelts)!=NGX_OK)
	{
	...
	ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "$URI hashtable init OK, %d items", dlc->wlr_url_hash->size);
#endif
    }
  if (elements_body->nelts > 0)
    {
      ...
      hash_init.pool           = cf->pool;
      hash_init.temp_pool      = NULL;
      if (ngx_hash_init(&hash_init, (ngx_hash_key_t*) elements_body->elts, elements_body->nelts)!=NGX_OK)
	{
  ...
  if (elements_vars->nelts > 0)
    {
      ...
      hash_init.pool           = cf->pool;
      hash_init.temp_pool      = NULL;
      if (ngx_hash_init(&hash_init, (ngx_hash_key_t*) elements_vars->elts, elements_vars->nelts)!=NGX_OK)
	{
	...
	ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "$ARGS hashtable init OK, %d items final, %d items in list", dlc->wlr_args_hash->size,
			   elements_vars->nelts);
#endif
    }
  if (elements_headers->nelts > 0)
    {
      ...
      hash_init.pool           = cf->pool;
      hash_init.temp_pool      = NULL;
      if (ngx_hash_init(&hash_init, (ngx_hash_key_t*) elements_headers->elts, elements_headers->nelts)!=NGX_OK)
	{
