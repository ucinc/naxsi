...
static char		*ngx_http_dummy_read_main_conf(ngx_conf_t *cf, ngx_command_t *cmd, 
						       void *conf);
static ngx_int_t	ngx_http_dummy_init(ngx_conf_t *cf);
static char		*ngx_http_dummy_read_conf(ngx_conf_t *cf, ngx_command_t *cmd,
						  void *conf);
static void		*ngx_http_dummy_create_loc_conf(ngx_conf_t *cf);
static char		*ngx_http_dummy_merge_loc_conf(ngx_conf_t *cf, void *parent,
						       void *child);
...
/* NOTE/WARNING : This function wasn't tested correctly. 
 Actually, we shouldn't merge anything, as configuration is specific 'per' location ? */
static char *ngx_http_dummy_merge_loc_conf(ngx_conf_t *cf, void *parent, 
					   void *child)
{
				      ...
				      void *conf)
{
  ...
  if (cf)
    {
      value = cf->args->elts;
      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "TOP READ CONF %V %V", &(value[0]), &(value[1]));  
    }
#endif
  if (!alcf || !cf)
    return (NGX_CONF_ERROR); 
  ...
  if (!ngx_strcmp(value[0].data, TOP_BASIC_RULE_T))
    {
#ifdef readconf_debug
      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "XX-TOP READ CONF %s", value[0].data);  
#endif
	...
	{
#ifdef readconf_debug
	  ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "pushing rule %d in whitelist rules", rule.rule_id);  
#endif
      ...
      else
	{
	  if (rule.br->headers)
	    {
#ifdef readconf_debug
	      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "pushing rule %d in header rules", rule.rule_id);  
#endif
	  ...
	  if (rule.br->body || rule.br->body_var)
	    {
#ifdef readconf_debug
	      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "pushing rule %d in body rules", rule.rule_id);  
#endif
	  ...
	  if (rule.br->url)
	    {
#ifdef readconf_debug
	      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "pushing rule %d in generic rules", rule.rule_id);  
#endif
	  ...
	  if (rule.br->args_var || rule.br->args)
	    {
#ifdef readconf_debug
	      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "pushing rule %d in GET rules", rule.rule_id);  
#endif
	  ...
	  if (rule.br->custom_location)
	    {
#ifdef readconf_debug
	      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "pushing rule %d in custom_location rules", rule.rule_id);  
#endif
  ...
  /* this should be moved in a function, plus this code is not safe. */
    ...
    {
#ifdef readconf_debug
      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "pushing rule %d in check rules", rule.rule_id);  
#endif
      i = 0;
      if (!alcf->check_rules)
	...
	i++;
#ifdef readconf_debug
      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "XX-special score in checkrule:%s from (%d)", value[1].data, atoi((const char *)value[1].data+i));
#endif
      // get the score
...
static ngx_int_t ngx_http_dummy_access_handler(ngx_http_request_t *r)
{
  ...
  if (ctx && ctx->ready && !ctx->over)
	...
	ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
		      "XX-dummy : Failed to get time");
      if (end - start > 0)
	ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "[MORE THAN 1MS] times : start:%l end:%l diff:%l",
		      start, end, (end-start));
      ctx->over = 1;
      if (ctx->block)
	{
