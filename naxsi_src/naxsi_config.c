...
#define MZ_SPECIFIC_URL_T "$URL:"
	  ...
	  if (tmp_ptr[0] == '$') 
		...
		if (!strncmp(tmp_ptr, MZ_POST_VAR_T, strlen(MZ_POST_VAR_T)))
		  {
		  ...
		  if (!strncmp(tmp_ptr, MZ_HEADER_VAR_T, strlen(MZ_HEADER_VAR_T)))
		    {
		    ...
		    if (!strncmp(tmp_ptr, MZ_SPECIFIC_URL_T, strlen(MZ_SPECIFIC_URL_T))) 
		      { 
